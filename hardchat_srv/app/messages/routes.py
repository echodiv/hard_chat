...
def read():
    ...
    messages = current_user.messages_received.order_by(
        Messages.timestamp.desc()).group_by('sender_id').paginate(
            page, current_app.config['POST_PER_PAGE'], False)

    next_url = url_for('messages.read', page=messages.next_num) \
        if messages.has_next else None
    prev_url = url_for('messages.read', page=messages.prev_num) \
        if messages.has_prev else None
